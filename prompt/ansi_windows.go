...
// inputRecord describes a generic event.
...
// AnsiReader is an io.Reader that converts windows key codes to the
...
// Read reads data from the input and converts windows key codes to ansi
// escapes that can be read over multiple Reads.
...
// shortFunction creates a small function code.
...
// AnsiWriter is an io.Writer that converts ansi escapes to the
// equivalent windows functionality.
...
// Write reads the buffer and filters out ansi escapes and converts them
// to the windows functionality. Escapes may be spread across multiple
// Writes.
...
// finish finishes an escape code and calls the parsing function. Afterwards
